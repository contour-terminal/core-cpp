...
void ProgressBar::render(TerminalOutput& output, Style const& filledStyle, Style const& emptyStyle) const
{
    ...
    for ([[maybe_unused]] auto const block: std::views::iota(0, std::max(0, std::min(fullBlocks, _width))))
    ...
    if (fullBlocks < _width && remainder > 0.0f)
    {
        ...
        for ([[maybe_unused]] auto const i:
             std::views::iota(fullBlocks + 1, std::max(fullBlocks + 1, _width)))
            output.writeText(EmptyChar, emptyStyle);
    }
    else
    {
        // Render empty blocks
        for ([[maybe_unused]] auto const i: std::views::iota(fullBlocks, std::max(fullBlocks, _width)))
            output.writeText(EmptyChar, emptyStyle);
    }
}
