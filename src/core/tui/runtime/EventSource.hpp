...
struct FdRegistration
{
    ...
    core::platform::NativeHandle fd = core::platform::InvalidHandle; ///< The watched native handle.
...
class FdRegistry
{
  public:
    ...
    [[nodiscard]] FdToken attach(core::platform::NativeHandle fd, FdInterest interest)
    {
...
class EventSource
{
  public:
    ...
    /// @param interest The readiness bits to start watching.
    /// @return A token naming the registration, or @c FdToken::invalid() on failure.
    [[nodiscard]] virtual FdToken attach(core::platform::NativeHandle fd, FdInterest interest) = 0;
...
template <>
struct hash<core::tui::runtime::FdToken>
{
    /// @param token The token to hash.
    /// @return The hash of the token's underlying id.
    [[nodiscard]] std::size_t operator()(core::tui::runtime::FdToken token) const noexcept
    {
