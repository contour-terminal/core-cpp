...
namespace detail
{
    ...
    template <typename T>
    core::async::Task<void> captureResult(core::async::Task<T> work, std::shared_ptr<std::optional<T>> slot)
    {
    ...
    inline core::async::Task<void> timeoutArm(TuiRuntime* runtime, std::chrono::milliseconds timeout)
    {
...
core::async::Task<std::optional<T>> withTimeout(TuiRuntime* runtime,
                                                core::async::Task<T> work,
                                                std::chrono::milliseconds timeout)
{
    auto slot = std::make_shared<std::optional<T>>();
    auto tasks = std::vector<core::async::Task<void>> {};
    tasks.reserve(2);
...
inline core::async::Task<bool> withTimeout(TuiRuntime* runtime,
                                           core::async::Task<void> work,
                                           std::chrono::milliseconds timeout)
{
    auto tasks = std::vector<core::async::Task<void>> {};
    tasks.reserve(2);
