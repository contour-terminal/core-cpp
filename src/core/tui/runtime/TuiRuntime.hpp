...
/// and timers over it. Flows (`core::async::Task`s) suspend on the awaitables the
...
class TuiRuntime
{
  public:
    ...
    explicit TuiRuntime(EventSource& source,
                        core::platform::IClock& clock = core::platform::defaultSteadyClock()) noexcept;
    ...
    template <typename T>
    T blockOn(core::async::Task<T> task)
    {
    ...
    void spawn(core::async::Task<void> task);

    /// @return The root cancellation source; `request_stop()` cancels every flow.
    [[nodiscard]] core::async::StopSource& rootStopSource() noexcept { return _rootStop; }
    ...
    [[nodiscard]] core::platform::IClock& clock() const noexcept { return _clock; }
    ...
    [[nodiscard]] WaitFdAwaiter waitReadable(core::platform::NativeHandle fd) noexcept;
    ...
    ///         @c OperationCancelled if the flow is cancelled while parked.
    [[nodiscard]] WaitFdAwaiter waitWritable(core::platform::NativeHandle fd) noexcept;
    ...
    [[nodiscard]] FdToken registerFdWaiter(core::platform::NativeHandle fd,
                                           FdInterest interest,
  ...
  private:
    ...
    core::platform::IClock& _clock;             ///< The injected monotonic time source.
    ...
    std::vector<core::async::Task<void>> _roots; ///< Keeps spawned background flows alive.
    core::async::StopSource _rootStop;           ///< Root cancellation source.
...
class DelayAwaiter
{
  ...
  private:
    std::optional<core::async::StopCallback<std::function<void()>>> _cancelReg;
    TuiRuntime& _runtime;
    std::chrono::steady_clock::time_point _deadline;
    core::async::StopToken _token;
};
...
class WaitFdAwaiter
{
  public:
    ...
    /// @param fd The native handle to wait on.
    /// @param interest The readiness to wait for (Read or Write).
    WaitFdAwaiter(TuiRuntime& runtime, core::platform::NativeHandle fd, FdInterest interest) noexcept:
    ...
    /// signal.
    [[nodiscard]] bool await_ready() const noexcept { return _fd == core::platform::InvalidHandle; }
