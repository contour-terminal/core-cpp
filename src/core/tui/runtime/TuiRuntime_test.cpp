...
#include <core/tui/runtime/PollEventSource.hpp>
#include <core/tui/runtime/TuiRuntime.hpp>
#include <core/tui/runtime/WithTimeout.hpp>
...
Task<int> awaitReadableOrCancel(TuiRuntime* runtime, core::platform::NativeHandle fd, int cancelSentinel)
{
    try
    {
...
Task<int> parkOnFdForever(TuiRuntime* runtime, core::platform::NativeHandle fd)
{
    co_await runtime->waitReadable(fd);
    co_return 0;
}
...
Task<void> waitReadableWithGuard(TuiRuntime* runtime, core::platform::NativeHandle fd, bool* destroyed)
{
    *destroyed = false;
    auto guard = UnwindFlag { destroyed };
    try
...
class ClockAdvancingSource: public MockEventSource
{
  public:
    ...
    core::tui::runtime::WaitOutcome wait(int timeoutMs) override
    {
...
TEST_CASE("EventSource fd registry hands out distinct tokens and reports readiness", "[EventSource]")
{
    ...
    auto const a = source.attach(core::platform::standardInput(), core::tui::runtime::FdInterest::Read);
    auto const b = source.attach(core::platform::standardOutput(), core::tui::runtime::FdInterest::Write);
...
{
    auto const invalid = core::tui::runtime::FdToken::invalid();
    REQUIRE_FALSE(static_cast<bool>(invalid));
    REQUIRE(invalid == core::tui::runtime::FdToken {});
}
...
TEST_CASE("waitReadable resumes when the registered fd becomes readable", "[TuiRuntime][fd]")
{
    ...
    // resolves the park.
    source.pushReadable(core::tui::runtime::FdToken { 1 });
    auto runtime = TuiRuntime { source };

    constexpr auto Cancelled = -1;
    auto const result =
        runtime.blockOn(awaitReadableOrCancel(&runtime, core::platform::standardInput(), Cancelled));
...
TEST_CASE("waitReadable on an invalid fd resolves immediately as cancelled", "[TuiRuntime][fd]")
{
    ...
    auto const result =
        runtime.blockOn(awaitReadableOrCancel(&runtime, core::platform::InvalidHandle, Cancelled));
...
TEST_CASE("waitReadable resolves over a real SystemPipe via PollEventSource", "[TuiRuntime][fd][poll]")
{
    ...
    auto source = core::tui::runtime::PollEventSource {};
    auto runtime = TuiRuntime { source };

    auto readByte = [](TuiRuntime* rt, core::platform::SystemPipe* p) -> Task<char> {
...
TEST_CASE("withTimeout returns the work's value when it finishes first", "[TuiRuntime][timeout]")
{
    ...
    auto result = runtime.blockOn(
        core::tui::runtime::withTimeout(&runtime, produceValue(42), std::chrono::seconds { 10 }));
          ...
          "[TuiRuntime][timeout][poll]")
{
    ...
    auto result = runtime.blockOn(core::tui::runtime::withTimeout(
...
TEST_CASE("Destroying the runtime unwinds a flow parked on waitReadable", "[TuiRuntime][fd]")
{
    ...
    {
        auto runtime = TuiRuntime { source };
        runtime.spawn(waitReadableWithGuard(&runtime, core::platform::standardInput(), &destroyed));
