...
#include <core/tui/runtime/posix/PollHelpers.hpp>
...
WaitOutcome TerminalEventSource::waitForReadiness(int timeoutMs)
{
    ...
    auto resizeIndex = -1;
    if (auto const resizeFd = input.resizeNativeHandle(); resizeFd != core::platform::InvalidHandle)
    {
    ...
    auto signalIndex = -1;
    if (_signalFd != core::platform::InvalidHandle)
    {
    ...
    if (ready(signalIndex))
        ...
        if (core::platform::SignalHandler::processSignalFd())
