...
bool KqueueEventSource::applyInterest(platform::NativeHandle fd,
                                      FdInterest interest,
                                      FdToken token) const noexcept
{
    if (_kq < 0 || fd == platform::InvalidHandle)
        return false;
...
void KqueueEventSource::dropFilters(platform::NativeHandle fd) const noexcept
{
    if (_kq < 0 || fd == platform::InvalidHandle)
        return;
...
FdToken KqueueEventSource::attach(platform::NativeHandle fd, FdInterest interest)
{
    if (_kq < 0 || fd == platform::InvalidHandle)
