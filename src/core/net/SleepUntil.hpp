...
/// resolves INLINE — `await_ready()` answers true and the coroutine never suspends, so nothing is
/// filed with a loop that could not take it back.
///
...
/// @return An awaitable that resumes at @p deadline; it never suspends when @p loopOrNull is null
