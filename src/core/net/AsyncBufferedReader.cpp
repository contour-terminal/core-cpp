...
async::Task<std::expected<std::string, NetError>> AsyncBufferedReader::readUntil(std::string_view delimiter)
{
...
async::Task<std::expected<std::string, NetError>> AsyncBufferedReader::readExactly(std::size_t count)
{
...
async::Task<std::expected<void, NetError>> AsyncBufferedReader::fill()
{
...
async::Task<IoResult> appendReadChunk(ISocket* socket, std::vector<std::byte>* buffer)
{
