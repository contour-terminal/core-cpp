...
std::expected<std::unique_ptr<UnixListener>, NetError> UnixListener::bind(EventLoop& loop,
                                                                          ...
                                                                          int backlog)
{
    // A path with no directory component names a socket in the CURRENT directory
    // (`--socket=contour.sock`), whose parent_path() is empty — and `mkdir("")` fails with ENOENT,
    // not EEXIST, so hardening it unconditionally refused such a path before it was ever tried, with
    // a diagnostic naming a directory operation the user never asked for. Nor is the working
    // directory ours to create or re-permission: it is the caller's own, routinely group- or
    // world-readable, so demanding 0700 of it would reject the same paths for a different reason.
    // The socket file's own 0600 mode below is what gates access to it either way.
    if (auto const directory = path.parent_path(); !directory.empty())
        if (auto hardened = ensureOwnedPrivateDirectory(directory); !hardened)
            return std::unexpected(hardened.error());
...
async::Task<AcceptResult> UnixListener::accept()
{
