...
void sendWithFds(int socketFd, std::string_view payload, std::span<int const> fds)
{
    ...
    if (!fds.empty())
    {
        ...
        if (cmsg == nullptr)
            FAIL("the control buffer has no room for a header");
        else
        {
...
TEST_CASE("the default readWithFd never yields a descriptor", "[net][fdpass]")
{
    ...
    auto scenario =
        [](core::net::EventLoop* eventLoop, core::net::ISocket* a, core::net::ISocket* b) -> Task<void> {
...
TEST_CASE("a split socket reads one half and writes the other", "[net][fdpass]")
{
    ...
    auto readHalf = core::net::adoptFd(pair.loop, inbound[0]);
    auto writeHalf = core::net::adoptFd(pair.loop, outbound[1]);
    REQUIRE(readHalf.has_value());
    REQUIRE(writeHalf.has_value());
    auto split = core::net::combineHalves(std::move(*readHalf), std::move(*writeHalf));
...
TEST_CASE("a split socket forwards an fd received on its read half", "[net][fdpass]")
{
    ...
    auto writeHalf = core::net::adoptFd(pair.loop, outbound[1]);
    REQUIRE(writeHalf.has_value());
    auto split = core::net::combineHalves(std::move(pair.ours), std::move(*writeHalf));
