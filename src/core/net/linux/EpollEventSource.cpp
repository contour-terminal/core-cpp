...
bool EpollEventSource::applyInterest(platform::NativeHandle fd,
                                     FdInterest interest,
...
FdToken EpollEventSource::attach(platform::NativeHandle fd, FdInterest interest)
{
    if (_epollFd < 0 || fd == platform::InvalidHandle)
