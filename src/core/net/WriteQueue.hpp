...
class WriteQueue
{
  public:
    ...
    [[nodiscard]] async::Task<void> flushThenClose();
    ...
    [[nodiscard]] async::Task<void> waitUntilBacklogBelow(std::size_t watermark,
  ...
  private:
    ...
    static async::Task<void> drain(std::shared_ptr<State> state);
