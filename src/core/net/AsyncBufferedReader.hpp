...
class AsyncBufferedReader
{
  public:
    ...
    [[nodiscard]] async::Task<std::expected<std::string, NetError>> readLine();
    ...
    ///         the socket's own read error.
    [[nodiscard]] async::Task<std::expected<std::string, NetError>> readUntil(std::string_view delimiter);
    ...
    [[nodiscard]] async::Task<std::expected<std::string, NetError>> readExactly(std::size_t count);
  ...
  private:
    ...
    [[nodiscard]] async::Task<std::expected<void, NetError>> fill();
