...
struct FdRegistration
{
    ...
    platform::NativeHandle fd = platform::InvalidHandle; ///< The watched native handle.
...
class FdRegistry
{
  public:
    ...
    [[nodiscard]] FdToken attach(platform::NativeHandle fd, FdInterest interest)
    {
...
class EventSource
{
  public:
    ...
    /// @param interest The readiness bits to start watching.
    /// @return A token naming the registration, or @c FdToken::invalid() on failure.
    [[nodiscard]] virtual FdToken attach(platform::NativeHandle fd, FdInterest interest) = 0;
...
template <>
struct hash<core::net::FdToken>
{
    /// @param token The token to hash.
    /// @return The hash of the token's underlying id.
    [[nodiscard]] std::size_t operator()(core::net::FdToken token) const noexcept
    {
