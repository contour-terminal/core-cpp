...
class FakeSocket final: public core::net::ISocket
{
  public:
    ...
    Task<core::net::IoResult> read(std::span<std::byte> buffer) override
    {
        if (_chunks.empty())
        {
            if (_failure.has_value())
                co_return std::unexpected(core::net::makeNetError(*_failure, 0, "injected failure"));
...
Task<void> readOneLine(AsyncBufferedReader* reader,
                       std::string* line,
...
TEST_CASE("readLine reports EOF and drops an unterminated tail", "[net][reader]")
{
    ...
    auto firstError = std::optional<core::net::NetError> {};
    loop.blockOn(readOneLine(&reader, &first, &firstError));
    REQUIRE(first == "complete");

    auto second = std::string {};
    auto secondError = std::optional<core::net::NetError> {};
...
TEST_CASE("readLine works over a real transport through the reactor", "[net][reader][poll]")
{
    ...
    auto writeThenRead =
        [](core::net::ISocket* writer, core::net::ISocket* peer, std::string* out) -> Task<void> {
