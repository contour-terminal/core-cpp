...
EventLoop::EventLoop(EventSource& source, platform::IClock& clock): _source(source), _clock(clock)
{
...
FdToken EventLoop::registerFdWaiter(platform::NativeHandle fd,
                                    FdInterest interest,
...
std::coroutine_handle<> EventLoop::dropParkedWaiter(FdToken token) noexcept
{
    ...
    auto const entry = std::ranges::find_if(
        first, last, [token](auto const& candidate) { return candidate.second == token; });
    if (entry != last)
