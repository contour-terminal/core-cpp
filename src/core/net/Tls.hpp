...
///
/// @p expectedHostName additionally binds the certificate to the host that was asked for. A
/// chain check alone only proves the certificate was signed by the pinned CA — NOT that it was
/// issued for this peer, so any certificate that CA ever signed would be accepted for any
/// endpoint. Empty skips the name check (and it is moot without a CA, where nothing is verified).
///
/// The name is matched against the certificate's subjectAltName / CN as X509_VERIFY_PARAM does,
/// so an IP-literal host only matches a certificate that carries that IP. No SNI is sent: the
/// daemon serves exactly one certificate per listener, and SNI must not carry an IP literal.
///
...
///
/// Lives in `net` because this is where the vetted primitive (`CRYPTO_memcmp`) is reachable:
/// OpenSSL is a PRIVATE dependency here, and this signature keeps it that way — no OpenSSL type
/// crosses the header.
///
/// The LENGTHS are compared normally, which is deliberate and not a leak worth closing: a
/// length difference is already observable in the frame that carried the secret.
///
