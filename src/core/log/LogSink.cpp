...
namespace
{
                                     ...
                                     std::string_view tag)
    {
        ...
        for (auto const [index, line]: core::views::enumerate(core::split(message.text(), '\n')))
        {
            if (index == 0)
                appendPrefix();
            else
...
{
    // "-" is the established spelling for a standard stream.
    if (spec.empty() || spec == "-")
        return std::nullopt;
...
std::vector<std::string> unmatchedFilters(std::string_view filterString)
{
    ...
    for (auto const pattern: core::split(filterString, ','))
    {
        if (pattern.empty())
            continue;
           ...
           } }
{
    ...
    // whatever core::cli::App already set up. Categories constructed AFTER this point are not
