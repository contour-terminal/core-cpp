...
constexpr void encode(uint8_t ch, Alphabet const& alphabet, EncoderState& state, Sink const& s)
{
    ...
    s(alphabet[(input[0] >> 2) & 0x3F],
      alphabet[static_cast<uint8_t>(((input[0] & 0x03) << 4) | (static_cast<uint8_t>(input[1] & 0xF0) >> 4))],
      alphabet[static_cast<uint8_t>(((input[1] & 0x0F) << 2) | (static_cast<uint8_t>(input[2] & 0xC0) >> 6))],
...
constexpr void finish(Alphabet const& alphabet, EncoderState& state, Sink const& s)
{
    ...
    switch (state.modulo)
    {
        case 2: {
            s(alphabet[(input[0] >> 2) & 0x3F],
              alphabet[static_cast<uint8_t>(((input[0] & 0x03) << 4)
                                            | (static_cast<uint8_t>(input[1] & 0xF0) >> 4))],
...
std::string encode(Iterator begin, Iterator end, Alphabet alphabet)
{
    ...
    for (auto const ch: std::ranges::subrange(begin, end))
        encode(static_cast<uint8_t>(ch), alphabet, state, flusher);
...
size_t decode(Iterator begin, Iterator end, IndexTable const& indexmap, Output output)
{
    ...
    while (nprbytes > 4)
    {
        *out++ = static_cast<char>(index(input + 0) << 2 | index(input + 1) >> 4);
        *out++ = static_cast<char>(index(input + 1) << 4 | index(input + 2) >> 2);
        *out++ = static_cast<char>(index(input + 2) << 6 | index(input + 3));
