# Brief for Task C6: contour vendors core-cpp v0.2.0

The plan text comes first, verbatim. The rulings after it override the plan wherever the two disagree.
...
1. Rulings 1-8 of D:/core-cpp/.superpowers/sdd/2026-09-18-core-cpp/task-C1-brief.md apply, with these substitutions:
   - repo: contour-terminal/contour
   - main checkout: D:\contour (never modified)
   - worktree: D:\contour-worktrees\core-cpp, on branch build/vendor-core-cpp from origin/master
2. **Vendor v0.2.0, not v0.1.0.** Use core-cpp's own `cmake/CoreCppVendor.cmake` from a core-cpp checkout at the tag (`git -C D:/core-cpp worktree add --detach D:/core-cpp-wt-contour v0.2.0`), with `-DREF=v0.2.0`. v0.2.1 comes shortly, and re-vendoring it later is one sync command plus a MANIFEST check. Say so in the PR body.
3. The vendored modules and options follow the plan (base, log, cli, platform, async, net, testing). The module is `async`, not `coro`: pass `MODULES=base;log;cli;platform;async;net;testing`. TLS ON, TUI OFF, FETCH_DEPS following CONTOUR_USE_CPM.
4. This PR must stay a **draft** until the endo (#187) and tuidu (#13) PRs have merged, because both fetch from contour master today. Say so at the top of the PR body.
5. The offline-build check matters for distro packagers: `-DCONTOUR_USE_CPM=OFF` fetches nothing. Show the configure log.
6. Keep scratch files under scratchpad\C6\ only.
