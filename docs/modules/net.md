...
the `ConnectEx` dial over it, which carry AF_UNIX too; the loopback pair). No file there guards
itself with an `#ifdef` of its platform. contour's `PollEventSource.cpp` was split along its
`#ifdef` into `posix/PollBackend.cpp` and a Windows half, the WFMO backend, which 0.5.0 removed
with its readiness sockets; `makeSocketPair()` is split into `testing/posix/` and
`testing/windows/`. A Windows socket factory refuses a loop whose backend lends no completion port
with `NetErrorCode::Unsupported`.
