...
| `CORE_CPP_INSTALL` | ON when top-level | Install core-cpp's targets and export them as the CMake package `core-cpp` ([Installing](install.md)). A parent whose own export links core-cpp's targets turns it on |
