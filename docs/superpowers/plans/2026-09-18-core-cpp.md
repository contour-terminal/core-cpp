...
- The modules are `base`, `log`, `cli`, `platform`, `coro`, `net`, `tui` and `testing`.
...
WebAssembly: the base, log, cli, coro and testing modules, plus subsets of platform and net, build and pass their tests under single-threaded Emscripten (no pthreads, emsdk 3.1.56 and latest). An event-loop backend driven by the host (the browser event loop) lets timers and coroutines run in the browser; morph's WebAssembly clients depend on this.
...
- Naming: types CamelCase, functions/methods/variables camelBack, private members `_x`, no `k`/`g_` prefixes, constants CamelCase.
...
- Fallible APIs return `std::expected<T, E>`, with monadic chaining preferred. The only exception type is `core::coro::OperationCancelled`.
...
- The WebAssembly subset (Part I §1) must build and pass tests under single-threaded Emscripten with emsdk **3.1.56** and latest.
  ...
  - Code there sticks to what libc++ 18/19 supports: check `__cpp_lib_*` before using newer library features.
...
- [ ] Move the `<stop_token>` probe into `core::coro` as an INTERFACE usage requirement (`-fexperimental-library` when `__cpp_lib_jthread` is missing; logic from `D:\contour\CMakeLists.txt:97-137`). It must be a no-op when the parent already set it, and it must work on emsdk 3.1.56's libc++. Expected: the CI `emscripten` job runs `core-cpp.coro` under node on both emsdk versions.
...
- Split the `core::tui_output` leaf (TerminalOutput, SgrBuilder, SyncGuard, TerminalProtocols, CursorShape, Error, `platform/PosixIO`, `Win32Utf`); verify it includes neither libunicode nor coro.
...
- [ ] **Step 3: consumer-smoke.**
  ...
  - (b) **Vendored consumer.** Export with `MODULES=base;log;cli;platform;coro;net;testing`, then `add_subdirectory(... SYSTEM EXCLUDE_FROM_ALL)` with `CORE_CPP_FETCH_DEPS OFF`, `CORE_CPP_WITH_TUI OFF`, `CORE_CPP_WITH_TLS ON`. It runs inside `docker run --network none` using an image with the toolchain.
  - (c) **WebAssembly consumer, mirroring morph.**
    - `tests/consumer-wasm` is a CPM consumer built with `emcmake` (emsdk 3.1.56).
    - It links only `core::base core::coro core::net`, with `CORE_CPP_WITH_TUI OFF` and `CORE_CPP_FETCH_DEPS OFF`.
    ...
    - Until B5 lands it only builds base and coro; the task that adds B5 extends it.
...
## Phase B — Merge fastcached Async/Net into core::coro / core::net
...
- Resolve the morph follow-up issue from Task C8, Step 6: executors and strand, logger, FileIoOps, the DateTime clock seam, and `morph::net` on Windows. Graduate morph's strand into `core::coro` once a second consumer needs one.
