...
- [ ] **Step 7: Windows dialog canary.** Register it `WILL_FAIL` + `TIMEOUT 60` on WIN32 (superseded by a48e727: every canary is judged by a marker through `PASS_REGULAR_EXPRESSION`, never `WILL_FAIL`). Run `ctest --preset cl-debug -R canary`. Expected: PASS (the canary died instead of hanging).
...
- [ ] Tests first:
  ...
  - The read-slot, write-slot and empty-read-buffer canaries (Debug, label `canary`; judged by markers since a48e727, not `WILL_FAIL`).
