...
| `src/core/coro/Task_test.cpp` | contour-terminal/contour | `src/coro/Task_test.cpp` | `6777ff05014f8ff163b071e8b0e942830119db80` | no `NOLINT`; the deep-chain case is skipped under Emscripten without `-mtail-call` and for GCC without `__OPTIMIZE__`, where symmetric transfer is not a tail call (core-cpp#15); also built over the `StopToken` fallback (`core-cpp.coro-fallback`) |
